...
public class SortCommandHandler implements CommandHandler {
    ...
    public void handle(Message message) {
        ...
        if (sortedUsers.size() > 0) {
            jokeBot.sendMessage(message, messageId, "Топ (5) хуевых шутников сегодня: ");
            if (sortedUsers.size() < 6)
                n = sortedUsers.size();
            else
                n = 5;
            for (int i = 0; i < n; i++) {
                jokeBot.sendMsg(message, allUsers.get(sortedUsers.get(i)).getFirstName() + " " + allUsers.get(sortedUsers.get(i)).getLastName() +
        ...
        if (sortedUsersOverZero.size() > 0) {
            jokeBot.sendMessage(message, messageId, "Топ (5) нормальных шутников сегодня: ");
            ...
            if (sortedUsersOverZero.size() < 6)
                ...
                n = 5;
            for (int i = 0; i < n; i++) {
                jokeBot.sendMsg(message, allUsers.get(sortedUsersOverZero.get(tempI)).getFirstName() + " " + allUsers.get(sortedUsersOverZero.get(tempI)).getLastName() +

...
public class DislikeCommandHandler implements CommandHandler {
    ...
    public static final String DISLIKE_CMD = "-";
    ...
    public boolean canHandle(Message message) {
        return message.isReply() && DISLIKE_CMD.equals(message.getText());
    }
        ...
        if (chatMap.containsKey(chatId)) { //check for the chat in chatMap
            if (chatMap.get(chatId).containsKey(newUser)) {
                if (chatMap.get(chatId).get(newUser).getReactedUser().contains(joke.getReactedUser()) &&
                    ...
                    jokeBot.sendMessage(chatId, messageId, "Уже минусовал это сообщение, хватит");
                } else {
